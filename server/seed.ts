...
import { practices, weeklyReports, marketingData, cfoScripts, forecasts, actionItems, benchmarks, users, rdActivities, taxPlanning } from "@shared/schema";
...
export function seedDatabase() {
  ...
  // R&D seed for Summit Spine
  db.insert(rdActivities).values({
    practiceId: p1.id, taxYear: 2025,
    checkedActivities: JSON.stringify([
      "new_treatment_protocols", "equipment_testing", "outcome_tracking",
      "laser_therapy_protocols", "soft_tissue_techniques", "staff_training_rd"
    ]),
    qreWages: 48000, qreSupplies: 8400, qreContractors: 0,
    totalQre: 56400, estimatedCredit: 3666, stateCredit: 1128, totalCredit: 4794,
    annualGrossRevenue: 2522000,
    notes: "Dr. Chen actively develops and tests new treatment protocols combining ART and instrument-assisted soft tissue mobilization. Staff time documented via time-tracking logs.",
    status: "draft", updatedAt: new Date().toISOString(),
  }).run();

  // Tax planning seed
  db.insert(taxPlanning).values({
    practiceId: p1.id, taxYear: 2025, planTier: "comprehensive",
    annualCollections: 2340000, annualGrossRevenue: 2522000,
    entityType: "S-Corp",
    strategiesJson: JSON.stringify([
      { id: "s_corp_salary", name: "Reasonable S-Corp Salary Optimization", savings: 18400, priority: "high" },
      { id: "qbi_deduction", name: "Section 199A QBI Deduction", savings: 52000, priority: "high" },
      { id: "rd_credit", name: "R&D Tax Credit (Section 41)", savings: 4794, priority: "high" },
      { id: "retirement", name: "Solo 401(k) / Defined Benefit Plan", savings: 24500, priority: "medium" },
      { id: "augusta_rule", name: "Augusta Rule (Section 280A)", savings: 5600, priority: "medium" },
      { id: "roth_conversion", name: "Roth Conversion Window", savings: 8200, priority: "medium" },
      { id: "cost_segregation", name: "Cost Segregation Study (if owns building)", savings: 12000, priority: "low" },
    ]),
    estimatedSavings: 113494,
    rothConversionAmount: 45000, rothConversionSavings: 8200,
    qbiDeduction: 52000,
    augustaRuleDays: 14, augustaRuleSavings: 5600,
    notes: "Comprehensive review completed. Priority: implement S-Corp salary optimization and QBI deduction before year-end.",
    advisorNotes: "Client is highly receptive. R&D credit discovery was a major win — they had no idea they qualified. Schedule follow-up on cost segregation if they refinance.",
    status: "presented", updatedAt: new Date().toISOString(),
  }).run();
