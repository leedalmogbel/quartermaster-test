...
import { Users, AlertTriangle, CheckCircle2, DollarSign } from "lucide-react";
    ...
    <div className="p-6 space-y-5 max-w-screen-xl">
      ...
      <Card className="border-card-border">
        <CardHeader className="pb-0">
          ...
          <p className="text-xs text-muted-foreground">Monthly retainer and add-on pricing structure</p>
        </CardHeader>
        <CardContent className="pt-3 space-y-5">
          {/* Base Plans */}
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Base Plans (Monthly Retainer)</p>
            <div className="overflow-x-auto">
              <table className="w-full" data-testid="pricing-base-table">
                <thead>
                  <tr className="bg-[hsl(var(--sidebar-primary)/0.1)] border-b border-border">
                    <th className="text-left px-4 py-2.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Tier</th>
                    <th className="text-left px-4 py-2.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Collections Range</th>
                    <th className="text-right px-4 py-2.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Monthly Fee</th>
                    <th className="text-left px-4 py-2.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Includes</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    {
                      ...
                      range: "Under $500K/yr",
                      fee: "$497/mo",
                      includes: "QB Integration, Monthly CFO Script, P&L Dashboard, Forecasting",
                      tierClass: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border-blue-200",
                    },
                    {
                      tier: "Growth",
                      range: "$500K–$1.5M/yr",
                      fee: "$997/mo",
                      includes: "Everything in Starter + Weekly CFO Scripts, Google/FB Ads Attribution, Action Items",
                      tierClass: "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 border-green-200",
                    },
                    {
                      tier: "Enterprise",
                      range: "Over $1.5M/yr",
                      fee: "$1,997/mo",
                      includes: "Everything in Growth + Weekly Advisory Calls, Custom Forecasting, Priority Support",
                      tierClass: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border-amber-200",
                    },
                  ].map((row) => (
                    <tr key={row.tier} className="border-b border-border hover:bg-muted/20 transition-colors" data-testid={`pricing-row-${row.tier.toLowerCase()}`}>
                      <td className="px-4 py-3">
                        <Badge variant="outline" className={cn("text-[10px] font-semibold capitalize", row.tierClass)}>
                          {row.tier}
                        </Badge>
                      </td>
                      <td className="px-4 py-3 text-sm text-muted-foreground">{row.range}</td>
                      <td className="px-4 py-3 text-right">
                        <span className="text-sm font-bold text-foreground">{row.fee}</span>
                      </td>
                      <td className="px-4 py-3 text-xs text-muted-foreground max-w-sm">{row.includes}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {/* Add-Ons */}
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Add-Ons (Additional Monthly Fee on Top of Base Plan)</p>
            <div className="overflow-x-auto">
              <table className="w-full" data-testid="pricing-addons-table">
                <thead>
                  <tr className="bg-muted/40 border-b border-border">
                    <th className="text-left px-4 py-2.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Add-On</th>
                    <th className="text-right px-4 py-2.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Price</th>
                    <th className="text-left px-4 py-2.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Best For</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    {
                      name: "R&D Coaching",
                      price: "+$297/mo",
                      bestFor: "Practices with documented R&D activities (most qualify)",
                    },
                    {
                      name: "Tax Planning Lite",
                      price: "+$197/mo",
                      bestFor: "Practices under $500K collections — entity review, QBI, retirement basics",
                    },
                    {
                      name: "Tax Planning Comprehensive",
                      price: "+$397/mo",
                      bestFor: "Practices $500K+ collections — full strategy including R&D, Augusta Rule, Roth, cost segregation",
                    },
                  ].map((row) => (
                    <tr key={row.name} className="border-b border-border hover:bg-muted/20 transition-colors">
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-xs text-muted-foreground max-w-sm">{row.bestFor}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="p-4 rounded-lg bg-muted/30 border border-border">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Sample Bundled Packages</p>
            <div className="space-y-2">
              <div className="flex items-start justify-between gap-4">
                <p className="text-sm text-foreground">Growth + R&amp;D Coaching + Tax Planning Comprehensive</p>
                <span className="text-sm font-bold text-foreground tabular-nums shrink-0">$1,691/mo</span>
              </div>
              <div className="flex items-start justify-between gap-4">
                <p className="text-sm text-foreground">Enterprise + R&amp;D Coaching + Tax Planning Comprehensive</p>
                <span className="text-sm font-bold text-foreground tabular-nums shrink-0">$2,691/mo</span>
              </div>
              <div className="mt-3 pt-3 border-t border-border">
                <p className="text-sm font-semibold text-primary">
                  At 20 Growth clients with add-ons at $1,691/mo = $33,820/month recurring revenue
                </p>
              </div>
            </div>
          </div>
